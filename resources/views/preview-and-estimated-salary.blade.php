...
<div class="my-2 py-3">
    <p>
        <strong>
            En maskinell beregning av lønnsplasseringen viser lønnstrinn {{ $salaryPlacement +
            $application->competence_points }}.
        </strong>
    </p>
    <p>
...
<div class="border border-primary border-1 bg-info px-3 ">
    ...
    <div class="my-4">
        <form action="{{ route('export-as-xls') }}" method="get" id="salary_form"
            _="on submit set #sendEmail.innerHTML to 'Behandler. Sjekk din epost om noen mulutter...'">
            @csrf
            <div class="pe-4">
...
<div class="border border-primary border-1 bg-info px-3 my-4">
    ...
    <div class="my-4">
        ...
        <a href="#" class="btn btn-sm btn-success"
            _="on click remove .d-none from #beregning then wait 200ms then go to #beregning @if (!$application->email_sent) then confirm('Ikke glem å motta beregningen og lenken til dette skjemaet via e-post') @endif">
            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 32 32">
                <path fill="currentColor"
                    d="M11 19v2H5v-2h2v-5H5v-2h2v-1h2v8zm8 0h-4v-2h2c1.103 0 2-.897 2-2v-2c0-1.103-.897-2-2-2h-4v2h4v2h-2c-1.103 0-2 .897-2 2v4h6zm6-8h-4v2h4v2h-3v2h3v2h-4v2h4c1.103 0 2-.897 2-2v-6c0-1.103-.897-2-2-2M2 4v4h2V4h4V2H4a2 2 0 0 0-2 2m26-2h-4v2h4v4h2V4a2 2 0 0 0-2-2M4 28v-4H2v4a2 2 0 0 0 2 2h4v-2zm24-4v4h-4v2h4a2 2 0 0 0 2-2v-4z">
                </path>
            </svg>
    ...
    <div class="my-4">
        <a href="#" class="btn btn-sm btn-success"
            _="on click remove .d-none from #tidslinje then wait 200ms then go to #tidslinje  @if (!$application->email_sent) then confirm('Ikke glem å motta beregningen og lenken til dette skjemaet via e-post') @endif">
            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 2048 2048">
                <path fill="currentColor"
                    d="M256 640h640v128H256zm1024 512h512v128h-512zm768-896v1408H0V256zm-127 128h-514v384h-127V384H768v128H640V384H128v1153h512V896h128v641h512v-129h127v129h514zM897 896h639v128H897z">
                </path>
...
<div class="callout callout-primary d-none" id="beregning">
    ...
    <div class="m-2">
        <strong>Lønnsplassering med kompetansepoeng:</strong> {{ $salaryPlacement + $application->competence_points
        }}<br />
    </div>
</div>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="table-primary">
                        @if ($currentMonth >= $itemStart && $currentMonth <= $itemEnd) <span class="badge bg-primary"
            ...
            <thead>
                <tr>
                    ...
                    $itemEnd = strtotime($item['end_date']);
                    $currentMonth = strtotime($month);
                    if ($currentMonth >= $itemStart && $currentMonth <= $itemEnd) { $monthlySum +=$item['percentage']; }
                        @endphp @endforeach <td @class([ 'bg-danger'=> $monthlySum > 100,
            ...
            <thead>
                <tr>
                    ...
                    @php
                    $monthDifference =
                    \Carbon\Carbon::parse($timeline[0])->diffInMonths(\Carbon\Carbon::parse($timeline_adjusted[0]));
                    @endphp
                    @for ($i = 0; $i < $monthDifference; $i++) <th
                        class="table-primary border-dark border-bottom border-start-0">{{ $timeline[$i] }}</th>
                        @endfor
                ...
                <tr>
                    <td>{{ strlen($item['title']) > 30 ? substr($item['title'], 0, 30) . '...' : $item['title'] }}
                        ...
                        <button type="button" class="btn btn-sm btn-outline-success" data-bs-toggle="tooltip"
                            data-bs-placement="top" data-bs-title="{{ $item['comments'] }}">
                            info
                        </button>
                        @endif
                    </td>
                    @for ($i = 1; $i <= $monthDifference; $i++) <td class="table-primary">
                        </td>
                        @endfor
                        @foreach ($timeline_adjusted as $month)
                        @php
                        ...
                        @endphp
                        <td class="table-primary">
                            @if ($currentMonth >= $itemStart && $currentMonth <= $itemEnd) <span
                                class="badge bg-primary" @class([ 'education'=> $item['type'] === 'education',
                                'work' => $item['type'] === 'work',
                                ])>
                                {{ $item['percentage'] }}%
                                </span>
